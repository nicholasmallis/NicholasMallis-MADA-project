...
p4 <- complete %>% ggplot(aes(x=unemployment, y=pct_vax)) + geom_point(color="purple") + geom_smooth(method='lm', color="black") + labs(title= "Scatterplot of Unemployment Rate in County vs Percent Vaccinated", subtitle = ("All Counties, USA")) + xlab("Unemployment Rate in Countyy") +  ylab("Percent of County Vaccinated (12 Years +)") + theme_classic()
p4
...
```{r}
#first we load the usmap package
library(usmap)
library(ggplot2)
...
mapplot <- plot_usmap(data = map, values = "pct_vax") +
  scale_fill_continuous(name = "Percent Vaccinated", label = scales::comma, type = "viridis") + 
